...
class ResponseStats:
    ...
    def format_stats(self) -> str:
        ...
        return (f"⏱️  Response: {self.response_time:.2f}s | "
...
class ChatMessage:
    ...
    role: MessageRole
    content: str
    stats: Optional[ResponseStats] = None  # Statistics for generated responses
...
class ChatSession:
    ...
    def add_message(self, role: Union[MessageRole, str], content: str, stats: Optional[ResponseStats] = None):
    ...
    def get_last_response_stats(self) -> Optional[ResponseStats]:
...
class LLaMA3ChatInterface(ChatInterface):
    ...
    def encode_role(self, role: str) -> List[int]:
        ...
        return ([self.tokenizer.special_tokens["<|start_header_id|>"]] +
    ...
    def encode_message(self, message: ChatMessage) -> List[int]:
        ...
        return (self.encode_role(role_str) +
    ...
    def get_stop_tokens(self) -> List[int]:
        """LLaMA 3 stop tokens"""
        return [self.tokenizer.special_tokens["<|end_of_text|>"],
                self.tokenizer.special_tokens["<|eot_id|>"]]
...
def create_chat_interface(model_type: str, tokenizer) -> ChatInterface:
    ...
    elif model_type.startswith("gpt2") or model_type.startswith("gpt-2"):
...
def stream_generate_tokens(model_fn, initial_context, chat_interface: ChatInterface,
                          stats: ResponseStats, max_tokens: int = 100,

#!/usr/bin/env python3
import os, argparse, contextlib, sys
...
from chat_interface import (
    ...
    GPT2ChatInterface, create_simple_session
)
...
class GPT2:
  ...
  def generate_token(self, tokens_sequence, start_pos: int, temperature: float = 0.85):
    ...
    from tinygrad import Variable, GlobalCounters, Tensor
    GlobalCounters.reset()
    ...
    tok = self.model(tokens, Variable("start_pos", 1 if start_pos else 0, MAX_CONTEXT-1).bind(start_pos), temperature).item()
    return tok

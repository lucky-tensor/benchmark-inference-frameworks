...
# Add current directory to path for chat_interface import
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from chat_interface import (
    ChatSession, ChatMessage, MessageRole, ResponseStats,
    LLaMA3ChatInterface, create_simple_session
)

# Set custom download directory to match inference.py
...
def prefill(model, toks, start_pos=0, device_param=None):
  ...
  # Use global device if no parameter provided, otherwise use provided device
  model_device = device_param if device_param is not None else device
...
if __name__ == "__main__":
  ...
  parser.add_argument("--no_api", action="store_true", default=True, help="Disable the api and run a cli test interface")
  ...
  # Helper function to find model in default directory
  def find_model_in_default_dir(size: str) -> Optional[Path]:
    """Look for model files in ~/models directory based on size"""
    models_dir = Path.home() / "models"

    # Model directory mappings based on our download structure
    size_to_dir = {
      "1B": "llama3-1b-instruct",
      "8B": "llama3-8b-sfr",
      "70B": "DeepSeek-R1-Distill-Llama-70B",
      "405B": "llama3-405b"  # placeholder
    }
    ...
    # Look for different file types based on size
    if size == "1B":
      # Look for GGUF file
  ...
  if not args.model and not args.download_model:
    found_model = find_model_in_default_dir(args.size)
    if found_model:
      args.model = found_model
      print(f"Found model: {args.model}")
    else:
      print(f"No {args.size} model found in ~/models, will download...")
  ...
  if not args.no_api and not args.benchmark:
    ...
    # Interactive Q&A mode using generic chat interface
    ...
    while True:
      try:
        ...
        # Run prefill for user message + assistant role

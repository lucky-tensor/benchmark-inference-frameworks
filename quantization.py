"""
Quantization classes for model compression.
"""
    ...
    def quantize(tensors, device, scale_dtype=dtypes.float16, quantize_embeds=False):
        ...
        for name, v in tensors.items():
            if "feed_forward" in name or "attention.w" in name or (quantize_embeds and "tok_embeddings.weight" in name):
                ...
                int8_weight = (v.T / scale).T.round().cast(dtype=dtypes.int8)
...
class Int8Embedding:
    def __init__(self, vocab_size: int, embed_size: int):
        ...
        self.scale = Tensor.ones(vocab_size, dtype=dtypes.half)
...
def NF4Linear(block_size):
    ...
    class _NF4Linear:
        ...
        @staticmethod
        def quantize(
            state_dict: dict[str, Tensor], device, scale_dtype=dtypes.float16, quantize_embeds=False
        ) -> dict[str, Tensor]:

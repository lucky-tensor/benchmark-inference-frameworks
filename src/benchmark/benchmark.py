...
class PyTorchBackend(FrameworkBackend):
    ...
    def load_model(self, model_size: str, model_path: Path | None = None, **kwargs) -> Any:
        ...
        spec = importlib.util.spec_from_file_location("pytorch_backend", "pytorch-pure.py")
    ...
    def prepare_input(self, _model: Any, tokenizer: Any) -> tuple[Any, int]:
...
def run_benchmark(
    ...
    try:
        _ = backend.run_inference(model, input_data, start_pos)
    ...
    for i in range(iterations):
        ...
        try:
            ...
            print(
                f"{status_symbol} Iteration {i + 1:2d}: {iteration_time * 1000:6.2f}ms, "
                f"{tokens_per_second:6.1f} tok/s{warmup_text}"
            )

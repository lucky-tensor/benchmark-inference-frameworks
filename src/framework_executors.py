#!/usr/bin/env python3
"""
Framework-specific executor implementations for the benchmark system.

Each framework (TinyGrad, PyTorch variants, etc.) has its own executor class
that implements the FrameworkExecutor interface.
"""
...
class TinyGradExecutor(FrameworkExecutor):
    ...
    def load_model(self, bench_run: BenchRun) -> Any:
        ...
        sys.path.insert(0, str(current_dir / "llama"))  # For extra module imports
...
def create_executor(framework_name: str) -> FrameworkExecutor:
    ...
    executor_map = {
        "tinygrad": TinyGradExecutor,
        "pytorch-unoptimized": PyTorchUnoptimizedExecutor,
        "pytorch-inductor": PyTorchInductorExecutor,
        "pytorch-eager": PyTorchEagerExecutor,
        "pytorch-aot_eager": PyTorchAOTEagerExecutor,
    }

    if framework_name not in executor_map:
        available = ", ".join(executor_map.keys())
        raise ValueError(f"Unknown framework: {framework_name}. Available: {available}")

    return executor_map[framework_name]()

...
class Attention:
    def __init__(self, dim, n_heads, n_kv_heads=None, max_context=0, linear=nn.Linear, qk_norm=None):
    ...
    def __call__(self, x: Tensor, start_pos, freqs_cis: Tensor, mask=None) -> Tensor:
        if getenv("WQKV"):
...
class TransformerBlock:
    ...
    def __call__(self, x: Tensor, start_pos, freqs_cis: Tensor, mask=None):
...
def sample(logits: Tensor, temp: float, k: int, p: float, af: float, ap: float):
    ...
    if af or ap:
        if not hasattr(sample, "alpha_counter"):
            sample.alpha_counter = Tensor.zeros_like(logits, dtype=dtypes.int32).contiguous()
...
class Transformer:
    def __init__(
        ...
        self.output = (
            nn.Linear(dim, vocab_size, bias=False) if embedding == nn.Embedding else linear(dim, vocab_size, bias=False)
        )
        self.max_context = max_context
        self.freqs_cis = (
            precompute_freqs_cis(dim // n_heads, self.max_context * 2, rope_theta).contiguous().requires_grad_(False)
        )
    ...
    def forward(
        ...
        mask = (
            Tensor.full((1, 1, seqlen, start_pos + seqlen), float("-inf"), dtype=h.dtype, device=h.device).triu(
                start_pos + 1
            )
            if seqlen > 1
            else None
...
def convert_from_huggingface(
    weights: dict[str, Tensor], n_layers: int, n_heads: int, n_kv_heads: int, permute_layers: bool = True
):
